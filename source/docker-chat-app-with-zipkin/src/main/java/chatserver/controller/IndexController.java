...
public class IndexController {
    ...
    @Value("${CONCERT_APP_SERVICE_HOST}")
    private String concertAppHost;
    
    @Value("${CONCERT_APP_SERVICE_PORT}")
    private String concertAppPort;
	...
	private void retrieveWeatherData(@ModelAttribute Message message) {
		...
		Weather weather = restTemplate.getForObject("http://" + weatherAppHost + ":" + weatherAppPort + "/weather?place=" + message.getContent().split(" ")[1], Weather.class);
  ...
  private void retrieveConcertData(@ModelAttribute Message message) {
		...
		ConcertInfo concertInfo = restTemplate.getForObject("http://" + concertAppHost + ":" + concertAppPort + "/concerts?place=" + message.getContent().split(" ")[1], ConcertInfo.class);
